...
def concat_torch_distribution(item, axis=0):
    ...
    if isinstance(item[0], (CustomIndependent, Independent)):
        reinterpreted_batch_ndims = item[0].reinterpreted_batch_ndims
        independent_type = type(item[0])
        item = [_.base_dist for _ in item]

    if isinstance(item[0], Normal):
        loc = torch.cat([_.loc for _ in item], dim=axis)
        scale = torch.cat([_.scale for _ in item], dim=axis)
    elif isinstance(item[0], Categorical):
        logits = torch.cat([_.logits for _ in item], dim=axis)

    if isinstance(item[0], (ScaledTanhNormal, ScaledNormal)):
        ret = type(item[0])(loc, scale, item[0].scale_prior, item[0].bias_prior)
    elif isinstance(item[0], Normal):
        ret = type(item)(loc, scale)
    elif isinstance(item[0], Categorical):
        ret = type(item[0])(logits=logits)
    else:
        raise NotImplementedError(f"You need to implement concat operation for {type(item[0])}!")

    if reinterpreted_batch_ndims is not None:
        ret = independent_type(ret, reinterpreted_batch_ndims)
    return ret
...
def ops_single_torch_distribution(item, func, *args, **kwargs):
    ...
    if isinstance(item, (CustomIndependent, Independent)):
        reinterpreted_batch_ndims = item.reinterpreted_batch_ndims
        independent_type = type(item)
        item = item.base_dist

    if isinstance(item, Normal):
        loc = func(item.loc, *args, **kwargs)
        scale = func(item.scale, *args, **kwargs)
    elif isinstance(item, Categorical):
        logits = func(item.logits, *args, **kwargs)
        # print(item.probs, item.logits)
        # exit(0)

    if isinstance(item, (ScaledTanhNormal, ScaledNormal)):
        ret = type(item)(loc, scale, item.scale_prior, item.bias_prior)
    elif isinstance(item, Normal):
        ret = type(item)(loc, scale)
    elif isinstance(item, Categorical):
        return type(item)(logits=logits)
    else:
        raise NotImplementedError(f"You need to implement concat operation for {type(item)}!")
