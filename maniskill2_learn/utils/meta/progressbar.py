...
class ProgressBar:
    ...
    def update(self, num_tasks=1):
        ...
        if elapsed > 0:
            ...
            fps = float("inf")
        if self.task_num > 0:
            percentage = self.completed / float(self.task_num - self.init_tasks)
            eta = int(elapsed * (1 - percentage) / percentage + 0.5)
            msg = (
                f"\r[{{}}] {self.completed + self.init_tasks}/{self.task_num}, "
...
def track_parallel_progress(
    ...
    Args:
        ...
        initializer (None or callable): Refer to :class:`multiprocessing.Pool` for details.
        initargs (None or tuple): Refer to :class:`multiprocessing.Pool` for details.
        chunksize (int): Refer to :class:`multiprocessing.Pool` for details.
        bar_width (int): Width of progress bar.
        skip_first (bool): Whether to skip the first sample for each worker when estimating fps, since the
            initialization step may takes longer.
