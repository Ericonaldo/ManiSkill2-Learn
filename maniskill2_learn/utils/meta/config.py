...
class Config:
    ...
    Example:
        ...
        >>> cfg.filename
        "/home/kchen/projects/maniskill2_learn/tests/data/config/a.py"
        >>> cfg.item4
        'test'
        >>> cfg
        "Config [path: /home/kchen/projects/maniskill2_learn/tests/data/config/a.py]: "
    ...
    def _substitute_predefined_vars(filename, temp_config_name):
        """
        There are four predefined variables: fileDirname, fileBasename, fileBasenameNoExtension, fileExtname
        """
    ...
    def _file2dict(filename, use_predefined_variables=True):
        ...
        from .path_utils import check_files_exist
        ...
        with tempfile.TemporaryDirectory() as temp_config_dir:
            ...
            if filename.endswith(".py"):
                ...
                cfg_dict = maniskill2_learn.load(temp_config_file.name)
            # close temp file
            temp_config_file.close()
    ...
    def fromfile(filename, use_predefined_variables=True, import_custom_modules=True):
        ...
        if import_custom_modules and cfg_dict.get("custom_imports", None):
            from .module_utils import import_modules_from_strings
...
def merge_a_to_b(a=None, b=None):
    from copy import deepcopy

    if a is None and b is None:
        return None
    if b is None:
        return config_to_dict(deepcopy(a))
    if a is None:
        return config_to_dict(deepcopy(b))

    b = Config(deepcopy(b)) if isinstance(b, dict) else deepcopy(b)
    if isinstance(a, Config):
        a = a.dict()
    assert isinstance(b, Config) and isinstance(a, dict), f"{type(a)}, {type(b)}"
    b.merge_from_dict(a)
    return b.dict()
