...
def _convert_input_type_range(img):
    """Convert the type and range of the input image. It converts the input image to np.float32 type and range of [0, 1]."""
    img_type = img.dtype
...
def convert_color_factory(src, dst):
    ...
    convert_color.__doc__ = f"""Convert a {src.upper()} image to {dst.upper()} image.
    Args:
