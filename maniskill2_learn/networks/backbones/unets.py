...
class ConditionalUnet1D(ExtendedModule):  # (nn.Module):
    def __init__(
        ...
        if self.jump_diffusion:
            self.current_dim_embedder = nn.Linear(jump_diffusion_max_step, 512)
            self.jump_conv = nn.Sequential(
                nn.Conv1d(mid_dim, mid_dim, 3, padding=1),
                nn.ReLU(),
            )
            self.x0_dim_predictor = nn.Sequential(
                nn.AdaptiveAvgPool1d(1),
                Rearrange("b c 1 -> b c"),
                nn.Linear(mid_dim, jump_diffusion_max_step),
            )
            ...
            self.add_value_predictor = nn.ModuleList([])
            for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
                is_last = ind >= (len(in_out) - 1)
                self.add_value_predictor.append(
                    nn.ModuleList(
                        [
                            ConditionalResidualBlock1D(
                                dim_out * 2,
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.returns_condition:
            ...
            global_feature = torch.cat([global_feature, returns_embed], dim=-1)
        ...
        if self.jump_diffusion:
            jump_emb = self.jump_conv(x)
            index_logits = self.index_predictor(jump_emb)  # (B max_step)
            x0_dim_logits = self.x0_dim_predictor(jump_emb)  # (B max_step)

            jump_h = copy(h)
            for idx, (resnet, resnet2, upsample) in enumerate(self.add_value_predictor[:-1]):
                jump_emb = torch.cat((jump_emb, jump_h.pop()), dim=1)
                jump_emb = resnet(jump_emb, global_feature)
                if idx == len(self.add_value_predictor[:-1]) and len(h_local) > 0:
                    jump_emb = jump_emb + h_local[1]
                jump_emb = resnet2(jump_emb, global_feature)
                jump_emb = upsample(jump_emb)
            pred_add_value = self.add_value_predictor[-1](jump_emb).squeeze(-1)  # (B input_dim)
        ...
        if self.jump_diffusion:
            return x, index_logits, x0_dim_logits, pred_add_value
        else:
            return x
