...
class AttentionPooling(MultiHeadedAttentionBase):
    ...
    def get_atten_score(self, x):
        ...
        score = (
            torch.einsum("nij,bnkj->bnik", self.q, k) / self.sqrt_latent_dim
        )  # [B, NH, 1, NK]
        return score
...
class MultiHeadAttention(MultiHeadedAttentionBase):
    ...
    def get_atten_score(self, key, query):
        ...
        score = (
            torch.einsum("bnij,bnkj->bnik", q, k) / self.sqrt_latent_dim
        )  # [B, NH, NQ, NK]
        return score
...
class MultiHeadSelfAttention(MultiHeadAttention):
    def __init__(self, embed_dim, num_heads, latent_dim, dropout=None):
        super(MultiHeadSelfAttention, self).__init__(
            embed_dim, num_heads, latent_dim, dropout
        )
...
class MultiHeadSelfAttentionXL(MultiHeadedAttentionBase):
    ...
    def __init__(self, embed_dim, num_heads, latent_dim, u, v, dropout=None):
        ...
        self.w_q = nn.Parameter(
            torch.empty(num_heads, embed_dim, latent_dim)
        )  # [NH, E, EL]
        ...
        self.inv_freq = nn.Parameter(
            1 / ((1e4) ** (torch.arange(0.0, embed_dim, 2.0) / embed_dim)),
            requires_grad=False,
        )  # [E / 2]
        self._reset_parameters()
    ...
    def get_atten_score(self, x, history=None, all_x=None):
        if all_x is None:
            all_x = (
                x if history is None else torch.cat([history, x], dim=1)
            )  # [B, H + N, EL]
        ...
        AC = (
            torch.einsum("bnij,bnkj->bnik", AC_q, AC_k) / self.sqrt_latent_dim
        )  # [B, NH, NQ, NK]
        ...
        # print('BNH+N', B, N, HpN)
        freq = (
            torch.arange(HpN + N, device=x.device)[:, None] - HpN + 1
        ) * self.inv_freq  # [H + 2N, E / 2]   from -N - H + 1 to N
        rel_embed = torch.cat([freq.sin(), freq.cos()], dim=-1)  # [H + 2N, E]

        BD_k = torch.einsum("lj,njd->nld", rel_embed, self.w_kr)  # [NH, H + 2N, EL]
        BD = (
            torch.einsum("bnij,nkj->bnik", BD_q, BD_k) / self.sqrt_latent_dim
        )  # [B, NH, N, H + 2N]
