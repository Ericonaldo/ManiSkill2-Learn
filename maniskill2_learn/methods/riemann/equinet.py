...
class SE3SegNet(ExtendedModule):
    ...
    def forward(
        self, inputs, draw_pcd=False, random_drop=False, pcd_name=None, mask_part=False
    ):
        ...
        for i in range(len(xyz)):
            pos_weight = torch.nn.functional.softmax(
                feature[i].reshape(-1, 1), dim=0
            ).squeeze()
        ...
        if draw_pcd:
            ...
            for i in range(len(xyz)):
                save_pcd_as_pcd(
                    xyz[i],
                    backbone_output["given_graph"]["raw_node_feats"][i][:, :3],
                    save_file=f"pcd/seg/original_{pcd_name}_{i}.pcd",
                )
                save_pcd_as_pcd(
                    xyz[i],
                    pos_weight.unsqueeze(-1).repeat(1, 3) / torch.max(pos_weight),
                    save_file=f"pcd/seg/pos_heatmap_{pcd_name}_{i}.pcd",
                    draw_heatmap=True,
                )
    ...
    def forward(
        ...
        for i in range(bs):
            if draw_pcd:
                ...
                save_pcd_as_pcd(
                    inputs["xyz"][i],
                    inputs["rgb"][i],
                    save_file=f"pcd/mani/original_{pcd_name}_{i}.pcd",
                )

                gt_heatmaps.append(
                    get_heatmap(
                        inputs["xyz"][i], closest_point_idx, std_dev=0.015, max_value=1
                    ).to(self.pos_net.device)
                )
                save_pcd_as_pcd(
                    inputs["xyz"][i],
                    gt_heatmaps[-1].unsqueeze(-1).repeat(1, 3)
                    / torch.max(gt_heatmaps[-1]),
                    save_file=f"pcd/mani/gt_heatmap_{pcd_name}_{i}.pcd",
                    draw_heatmap=True,
                )
        ...
        if train_pos:
            ...
            for i in range(len(xyz)):
                if draw_pcd:
                    save_pcd_as_pcd(
                        xyz[i],
                        feature[i][:, 0].clone().unsqueeze(-1).repeat(1, 3)
                        / torch.max(feature[i][:, 0].clone()),
                        save_file=f"pcd/mani/pos_heatmap_{pcd_name}_{i}.pcd",
                        draw_heatmap=True,
                    )
        ...
        if draw_pcd:
            for i in range(len(xyz)):
                ...
                save_pcd_as_pcd(
                    xyz[i],
                    seg_output["given_graph"]["raw_node_feats"][i][:, :3],
                    save_file=f"pcd/mani/ball_{pcd_name}_{i}.pcd",
                )
        ...
        for i in range(bs):
            newdata = seg_pointcloud(
                xyz[i],
                xyz[i],
                reference_point=output_pos[i],
                distance=self.feature_point_radius,
                extra_data={"feature": feature[i]},
            )
