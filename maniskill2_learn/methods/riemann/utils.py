...
def voxel_filter(pcd, feature, voxel_size: float, coord_reduction: str = "average"):
    ...
    vox_idx = torch.div((pcd - mins), voxel_size, rounding_mode="trunc").type(
        torch.long
    )
    shape = vox_idx.max(dim=-2).values + 1
    raveled_idx = torch.tensor(
        np.ravel_multi_index(vox_idx.T.cpu().numpy(), shape.cpu().numpy()),
        device=device,
        dtype=vox_idx.dtype,
    )

    n_pts_per_vox = torch_scatter.scatter(
        torch.ones_like(raveled_idx, device=device),
        raveled_idx,
    ...
    color_vox = torch_scatter.scatter(
        feature,
        raveled_idx.unsqueeze(-1),
        dim=-2,
        dim_size=shape[0] * shape[1] * shape[2],
    )[nonzero_vox].squeeze(-2)
    color_vox /= n_pts_per_vox.unsqueeze(-1)

    if coord_reduction == "center":
        coord_vox = np.stack(
            np.unravel_index(
                nonzero_vox.cpu().numpy().reshape(-1), shape.cpu().numpy()
            ),
            axis=-1,
        )
        ...
        coord_vox = torch_scatter.scatter(
            pcd,
            raveled_idx.unsqueeze(-1),
            dim=-2,
            dim_size=shape[0] * shape[1] * shape[2],
...
def build_graph(
    ...
    for pcd_index in range(bs):
        ...
        if voxelize:
            current_pcd, current_feature = voxel_filter(
                current_pcd, current_feature, voxel_size=voxel_size
            )
    ...
    for i in range(len(degrees)):
        feat = batched_graph.ndata["attr"][
            :, start : start + channels[i] * ((2 * degrees[i]) + 1)
        ]
        node_feats[str(degrees[i])] = feat.reshape(
            feat.shape[0], channels[i], 2 * degrees[i] + 1
        )  # n, channel, 2l+1
