...
class SE3Demo(Dataset):
    def __init__(self, demo_dir, data_aug=False, device="cuda:0", aug_methods=[]):
        ...
        self.xyz = (
            torch.from_numpy(demo["xyz"][:, 0:20, ...])
            .float()
            .to(self.device)
            .reshape(-1, point_num, 3)
        )
        self.rgb = (
            torch.from_numpy(demo["rgb"][:, 0:20, ...])
            .float()
            .to(self.device)
            .reshape(-1, point_num, 3)
        )
        self.seg_center = (
            torch.from_numpy(demo["seg_center"][:, 0:20, ...])
            .float()
            .to(self.device)
            .reshape(-1, 3)
        )
        self.axes = (
            torch.from_numpy(demo["axes"][:, 0:20, ...])
            .float()
            .to(self.device)
            .reshape(-1, 9)
        )
...
def seg_pointcould_with_boundary(
    xyz, rgb, xyz_for_seg, x_min, x_max, y_min, y_max, z_min, z_max
):
    rgb = rgb[
        (xyz_for_seg[:, 0] >= x_min)
        & (xyz_for_seg[:, 0] <= x_max)
...
def random_dropout(xyz, rgb, remain_point_ratio=[0.5, 0.9]):
    desired_points = int(
        np.random.uniform(remain_point_ratio[0], remain_point_ratio[1]) * xyz.shape[0]
    )
...
def jitter(data, std=0.03):
    data["xyz"] = data["xyz"] + std * torch.randn(data["xyz"].shape).to(
        data["xyz"].device
    )
    return data
...
def random_dropping_color(data, drop_ratio=0.3):
    ...
    mask = np.random.choice(
        [0, 1], size=N, replace=True, p=[1 - drop_ratio, drop_ratio]
    )
    data["rgb"][np.where(mask)] = torch.tensor([0.0, 0.0, 0.0]).to(data["rgb"].device)

    return data


def color_jitter(data, std=0.005):
    data["rgb"] = torch.clamp(
        data["rgb"]
        + (torch.rand(data["rgb"].shape).to(data["xyz"].device) - 0.5) * 2 * std,
        0,
        1,
    )
    return data
...
def hsv_transform(
    ...
    sat_shift_range=[0.5, 1.5],
    val_shift_range=[0.5, 2],
):
    img_rgb = data["rgb"].T.unsqueeze(
        -1
    )  # [N, 3] -> [3, N] -> [3, N, 1], and the adjust functions requires [3, H, W]

    hue_shift = (
        np.random.random_sample() * (hue_shift_range[1] - hue_shift_range[0])
        + hue_shift_range[0]
    )
    sat_shift = (
        np.random.random_sample() * (sat_shift_range[1] - sat_shift_range[0])
        + sat_shift_range[0]
    )
    val_shift = (
        np.random.random_sample() * (val_shift_range[1] - val_shift_range[0])
        + val_shift_range[0]
    )
...
def get_heatmap(
    point_cloud, keypoint_index, distance="geodesic", max_value=10.0, std_dev=0.005
):
