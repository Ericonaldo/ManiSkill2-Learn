...
class ExtendedSequential(torch.nn.Sequential):
    ...
    def append_list(self, modules):
        assert isinstance(modules, torch.nn.Sequential) or isinstance(
            modules, (list, tuple)
        )
...
class EquivariantNet(ExtendedModule):
    ...
    def forward(self, inputs, given_graph=None, given_basis=None, **kwargs):
        ...
        if not given_graph:
            batch_graph, node_feats, edge_feats, pcds, raw_node_feats = build_graph(
                xyz,
                feature,
            ...
            raw_node_feats = given_graph[
                "raw_node_feats"
            ]  # raw node feats are flattened node_feats

        if not given_basis:
            # Compute bases in case they weren't precomputed as part of the data loading
            basis = get_basis(
                batch_graph.edata["rel_pos"],
                max_degree=self.max_degree,
        ...
        node_feats = self.graph_modules(
            node_feats, edge_feats, graph=batch_graph, basis=basis
        )

        output = node_feats[list(node_feats.keys())[0]].reshape(
            node_feats[list(node_feats.keys())[0]].shape[0], -1
        )
        for type_l in list(node_feats.keys())[1:]:
            output = torch.cat(
                [output, node_feats[type_l].reshape(node_feats[type_l].shape[0], -1)],
                dim=1,
            )
...
class SE3Backbone(ExtendedModule):
    ...
    def forward(self, inputs):
        ...
        given_graph, basis, feature = self.net(
            inputs
        )  # node_feats are the (voxelized) feature vector, and feature are feed-forward output
        ...
        return {
            ...
            "frame_related_states": (
                inputs["frame_related_states"]
                if "frame_related_states" in inputs.keys()
                else None
            ),
            "basis": basis,
