...
class GPTConfig:
    ...
    def __init__(self, **kwargs):
        ...
        self.block_size = (
            kwargs["block_size"] * 2
            if "+a" in kwargs["model_type"]
            else kwargs["block_size"]
        )
...
class CausalSelfAttentionWithHist(nn.Module):
    ...
    def forward(self, x):
        ...
        att = att.masked_fill(
            self.mask[:, :, :T, :T] == 0, -99999999
        )  # Masked attention

        att = F.softmax(att, dim=-1)
        att = torch.where(
            torch.isnan(att), torch.full_like(att, 0), att
        )  # replace nan with 0.
...
class KeyframeGPTWithHist(nn.Module):
    ...
    def __init__(
        ...
        p_size = (
            ...
            if "+a" in self.model_type
            else self.config.block_size
        )
        self.local_pos_emb = nn.Parameter(torch.zeros(1, p_size, self.config.n_embd))
        if use_first_state:
            self.first_local_pos_emb = nn.Parameter(
                torch.zeros(1, 1, self.config.n_embd)
            )
        ...
        if self.state_dim > 1000:
            self.state_encoder = MLP(
                self.state_dim, self.config.n_embd, hidden_dims=[512, 256]
            )
        else:
            self.state_encoder = MLP(
                self.state_dim, self.config.n_embd, hidden_dims=[256]
            )

        # Action embeddings.
        if "+a" in self.model_type:
            self.action_encoder = MLP(
                self.action_dim, self.config.n_embd, hidden_dims=[256]
            )
        ...
        if self.pose_only:
            if self.compatible:
                self.key_frame_state_predictor = MLP(
                    self.config.n_embd, self.pose_dim + 1, hidden_dims=[256, 256]
                )  # We only predict pose+timestep
            else:
                self.key_frame_state_predictor = MLP(
                    self.config.n_embd, self.pose_dim, hidden_dims=[256, 256]
                )  # We only predict pose
        else:
            self.key_frame_state_predictor = MLP(
                self.config.n_embd, self.pred_state_dim, hidden_dims=[256, 256]
            )
        self.key_frame_action_predictor = MLP(
            self.config.n_embd, action_dim + 1, hidden_dims=[256, 256]
        )  # Action + timestep diffrence
    ...
    def forward(
        self, states, timesteps, actions=None, first_state=None
    ):  # Time steps should be the same shape as states
        ...
        token_embeddings = torch.zeros(
            [B, self.block_size, self.config.n_embd],
            dtype=torch.float32,
        ...
        timesteps_rp = torch.repeat_interleave(
            timesteps[:, None], self.config.n_embd, dim=-1
        )
        global_pos_emb = torch.gather(
            global_pos_emb, 1, timesteps_rp.long()
        )  # BS x 1 x D
        local_pos_emb = (
            torch.repeat_interleave(self.local_pos_emb, 2, dim=1)
            if "+a" in self.model_type
            else self.local_pos_emb
        )
        if first_state is not None:
            local_pos_emb = torch.cat([self.first_local_pos_emb, local_pos_emb], dim=1)[
                :, : self.block_size
            ]
        ...
        if False:  # self.pose_only:
            ...
            key_state_preds = self.key_frame_state_predictor(
                key_state_act_preds[:, T:]
            )  # The next tokens after s history
            key_act_preds = None
        else:
            if (
                "+a" in self.model_type
            ):  # The next tokens after s+a history should be s,a,s,a,s,...
                key_state_preds = self.key_frame_state_predictor(
                    key_state_act_preds[:, T * 2 - 1 : self.block_size : 2]
                )  # The next tokens after s+a history
                key_act_preds = self.key_frame_action_predictor(
                    key_state_act_preds[:, T * 2 : self.block_size : 2]
                )
            else:  # The next tokens after s history, should be s,a,s,a,s,..
                key_state_preds = self.key_frame_state_predictor(
                    key_state_act_preds[:, T : self.block_size : 2]
                )  # The next tokens after s history
                key_act_preds = self.key_frame_action_predictor(
                    key_state_act_preds[:, T + 1 : self.block_size : 2]
                )
