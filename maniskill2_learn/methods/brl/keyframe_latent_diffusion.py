...
class KeyLatentDiffAgent(DiffAgent):
    ...
    def update_parameters(self, memory, updates):
        if not self.init_normalizer:
            ...
            self.action_normalizer.fit(data, last_n_dims=1, mode='limits', range_eps=1e-7)
        ...
        sampled_batch = memory.sample(batch_size, device=self.device, obs_mask=self.obs_mask, require_mask=True, action_normalizer=self.action_normalizer)
        ...
        actions = sampled_batch["actions"] # Need Normalize! (Already did in replay buffer)
        ...
        if self.train_keyframe_model:
            ...
            keyframe_obs_fea = keyframe_obs_fea[:,obs_mask,...][:,-1] # We only take the last step of the horizon since we want to train the key frame model
            ...
            keyframe_loss, info = self.keyframe_loss(masked_obs_fea, timesteps, actions, keyframe_obs_fea, keyframe_actions, keytime_differences, keyframe_masks)
            ret_dict.update(info)
            loss += keyframe_loss
