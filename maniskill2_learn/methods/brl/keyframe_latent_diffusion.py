...
from maniskill2_learn.utils.torch import build_optimizer, get_cuda_info, get_mean_lr
...
@BRL.register_module()
class KeyLatentDiffAgent(DiffAgent):
    def __init__(
        self,
        actor_cfg,
        keyframe_model_cfg,
        ...
        pretrained_obs_encoder_path=None,
        **kwargs,
    ):
        super().__init__(
            actor_cfg=actor_cfg,
        ...
        assert (
            pretrained_obs_encoder_path is not None
        ), "pretrained_obs_encoder_path should be provided"
        ...
        self.keyframe_model = KeyframeGPTWithHist(
            keyframe_model_cfg, state_dim, keyframe_model_cfg.action_dim
        )
    ...
    def load_pretrained_encoder(self):
        pretrained_dict = torch.load(
            self.pretrained_obs_encoder_path, map_location=self.device
        )
        self.obs_encoder.load_state_dict(pretrained_dict["obs_encoder"])
    ...
    def keyframe_loss(
        ...
        pred_keyframe_states, pred_keyframe_actions, info = self.keyframe_model(
            states, timesteps, actions
        )  # (B, future_seq_len, act_dim+1)
        act_loss = (
            (pred_keyframe_actions[:, : keyframe_actions.shape[1]] - gt_actions) ** 2
        ).sum(-1)
        state_loss = (
    ...
    def update_parameters(self, memory, updates):
        if not self.init_normalizer:
            ...
            self.action_normalizer.fit(
                data, last_n_dims=1, mode="limits", range_eps=1e-7
            )
            data = memory.get_all("obs")
            data = self.obs_encoder(data)
            self.obs_normalizer.fit(data, last_n_dims=1, mode="limits", range_eps=1e-7)
        ...
        actions = sampled_batch[
            "actions"
        ]  # Need Normalize! (Already did in replay buffer)
        obs = sampled_batch["obs"]
        with torch.no_grad():
            obs = self.obs_encoder(obs)  # (B,H,fea_dim)
        obs_fea = self.normalizer.normalize(obs)
        masked_obs_fea = None

        act_mask, obs_mask, traj_mask = None, None
        if self.fix_obs_steps:
        ...
        if traj_mask is None:
            if self.obs_as_global_cond:
                ...
                obs_mask = obs_mask.unsqueeze(-1).repeat(
                    obs_fea.shape[0], 1, obs_fea.shape[-1]
                )
                traj_mask = torch.cat([obs_mask, act_mask], dim=-1)
                masked_obs_fea = obs_fea[:, obs_mask, ...]
            else:
        ...
        if self.train_diff_model:
            ...
            diff_loss, info = self.diff_loss(
                x=traj_data, masks=sampled_batch["is_valid"], cond_mask=traj_mask
            )
            ret_dict.update(info)
            loss += diff_loss

        if self.train_keyframe_model:
            keyframe_actions = sampled_batch[
                "keyframe_actions"
            ]  # Need Normalize! (Already did in replay buffer)
            keyframe_obs = sampled_batch["keyframe_obs"]
            with torch.no_grad():
                keyframe_obs_fea = self.obs_encoder(keyframe_obs)  # (B,N,fea_dim)
            ...
            actions = actions[:, obs_mask, ...]
            masked_obs_fea = masked_obs_fea
            keyframe_obs_fea = keyframe_obs_fea[:, obs_mask, ...][
                :, -1
            ...
            keyframe_loss, info = self.keyframe_loss(
                masked_obs_fea,
                timesteps,
                actions,
                keyframe_obs_fea,
