...
class KeyDiffAgent(DiffAgent):
    def __init__(
        ...
        **kwargs,
        ...
        if keyframe_model_type == "gpt":
            ...
            self.keyframe_model = KeyframeGPTWithHist(
                keyframe_model_cfg,
                keyframe_state_dim,
                keyframe_model_cfg.action_dim,
                pred_state_dim=keyframe_model_cfg.state_dim,
                use_first_state=use_ep_first_obs,
                pose_only=keyframe_pose_only,
                pose_dim=pose_dim,
                compatible=compatible,
            )
        elif keyframe_model_type == "bc":
            self.keyframe_model = MLP(
                input_dim=self.obs_feature_dim,
                output_dim=self.pose_dim + 1,
        ...
        if self.train_keyframe_model:
            if keyframe_model_type == "gpt":
                self.keyframe_optim = self.keyframe_model.configure_adamw_optimizers(
                    extra_model=self.keyframe_obs_encoder
                )
            elif keyframe_model_type == "bc":
                self.keyframe_optim = build_optimizer(
                    [self.keyframe_obs_encoder, self.keyframe_model], keyframe_optim_cfg
                )
                self.actor_optim = build_optimizer(
                    [self.model, self.obs_encoder], optim_cfg
                )  # Update using the same optimizer
        else:
        ...
        if keyframe_lr_scheduler_cfg is None:
            ...
            if self.keyframe_optim is not None:
                ...
                self.keyframe_lr_scheduler = build_lr_scheduler(
                    keyframe_lr_scheduler_cfg
                )
        ...
        if train_diff_model and train_keyframe_model and (not keyframe_state_only):
            assert id(self.obs_encoder.key_model_map["rgb"]) != id(
                self.keyframe_obs_encoder.key_model_map["rgb"]
            ), "same encoder?????"

        self.max_horizon = (
            self.action_seq_len - self.n_obs_steps
        )  # range [0, self.action_seq_len - self.n_obs_steps-1]
    ...
    def load_keyframe_model(self):
        ...
        if not isinstance(self, DDP):
            torch.nn.modules.utils.consume_prefix_in_state_dict_if_present(
                loaded_dict["state_dict"], prefix="module."
            )
            if "optimizer" in loaded_dict:
                torch.nn.modules.utils.consume_prefix_in_state_dict_if_present(
                    loaded_dict["optimizer"], prefix="module."
                )

        keyframe_state_dict = {
            _: loaded_dict["state_dict"][_]
            for _ in loaded_dict["state_dict"].keys()
            if "keyframe_model" in _
        }
        torch.nn.modules.utils.consume_prefix_in_state_dict_if_present(
            keyframe_state_dict, prefix="keyframe_model."
        )
        load_state_dict(self.keyframe_model, keyframe_state_dict)

        keyframe_obs_encoder_state_dict = {
            _: loaded_dict["state_dict"][_]
            for _ in loaded_dict["state_dict"].keys()
            if "keyframe_obs_encoder" in _
        }
        torch.nn.modules.utils.consume_prefix_in_state_dict_if_present(
            keyframe_obs_encoder_state_dict, prefix="keyframe_obs_encoder."
        )
    ...
    def keyframe_bc_loss(
        self, states, keyframe_states, keytime_differences, keyframe_masks
    ):
        ...
        if self.extra_dim > 0:
            gt_states = torch.cat(
                [
                    keyframe_states[
                        ..., -self.pose_dim - self.extra_dim : -self.extra_dim
                    ],
                    keytime_differences.unsqueeze(-1),
                ],
                dim=-1,
            )  # (B, max_key_frame_len, self.pose_dim+1)
        else:
            gt_states = torch.cat(
                [
                    keyframe_states[..., -self.pose_dim :],
                    keytime_differences.unsqueeze(-1),
                ],
                dim=-1,
            )  # (B, max_key_frame_len, self.pose_dim+1)
        ...
        info = {}
        pred_keyframe_states = self.keyframe_model(
            states
        )  # We expect states are obs features, pred_keyframe_states [B, self.pose_dim]

        state_loss = (
            (pred_keyframe_states[:, : keyframe_states.shape[1]] - gt_states) ** 2
        ).sum(-1)
    ...
    def keyframe_gpt_loss(
        ...
        if self.keyframe_pose_only:
            if self.extra_dim > 0:
                ...
                gt_states = keyframe_states[
                    ..., -self.pose_dim :
                ]  # (B, max_key_frame_len, self.pose_dim)
        else:
            gt_states = keyframe_states  # (B, max_key_frame_len, state_dim)
        gt_actions = torch.cat(
            [keyframe_actions, keytime_differences.unsqueeze(-1)], dim=-1
        )  # (B, max_key_frame_len, act_dim+1)

        pred_keyframe_states, pred_keyframe_actions, info = self.keyframe_model(
            obs, timesteps, actions, first_state=ep_first_state
        )  # (B, future_seq_len, act_dim+1)

        if (
            False
        ):  # self.pose_only: # We always choose to predict the action, so forget this path
            state_loss = (
                (pred_keyframe_states[:, : gt_states.shape[1]] - gt_states) ** 2
            ).sum(-1)
            masked_state_loss = state_loss * keyframe_masks
            ...
            act_loss = (
                (pred_keyframe_actions[:, : keyframe_actions.shape[1]] - gt_actions)
                ** 2
            ).sum(-1)
            state_loss = (
                (pred_keyframe_states[:, : gt_states.shape[1]] - gt_states) ** 2
            ...
            masked_loss = (
                masked_act_loss.sum(-1).mean() + masked_state_loss.sum(-1).mean()
            )
    ...
    def forward(self, observation, returns_rate=0.9, mode="eval", *args, **kwargs):
        ...
        if self.use_ep_first_obs and ("ep_first_obs" in observation):
            ...
            data = torch.cat(
                [
                    ep_first_obs["state"],
                    torch.zeros(
                        (ep_first_obs["state"].shape[0], self.action_dim),
                        dtype=states.dtype,
                        device=self.device,
                    ),
                ],
        ...
        if self.use_keyframe or self.keyframe_eval:
            ...
            if not self.keyframe_state_only:
                ...
                img_obs_fea = self.keyframe_obs_encoder(
                    tmp_observation, ep_first_obs_dict=ep_first_obs, img_fea_only=True
                )
            ...
            pred_keyframe_states, pred_keyframe_actions, info = self.keyframe_model(
                ...
                first_state=ep_first_obs_state,
            )

            if pred_keyframe_actions is not None:
                pred_keyframe_actions, pred_keytime_differences = (
                    pred_keyframe_actions[:, :, :-1],
                    pred_keyframe_actions[:, :, -1],
                )  # split keyframe and predicted timestep
                pred_keyframe = pred_keyframe_actions[
                    :, : self.pred_keyframe_num
                ]  # take the first key frame for diffusion
                if self.diffuse_state:
                    if (
                        pred_keyframe_states.shape[-1] == self.pose_dim + 1
                    ):  # Old problem, for compatibility
                        pred_keyframe_states, tmp_pred_keytime_differences = (
                            pred_keyframe_states[:, :, :-1],
                            pred_keyframe_states[:, :, -1],
                        )  # split keyframe and predicted timestep
                    pred_keyframe_states = pred_keyframe_states[
                        :, : self.pred_keyframe_num
                    ]

                    if self.keyframe_pose_only and (
                        pred_keyframe_states.shape[-1] < self.state_dim
                    ):
                        assert (
                            pred_keyframe_states.shape[-1] == self.pose_dim
                        ), "what are you predicting? pose should be {} but your prediction is {}".format(
                            self.pose_dim, pred_keyframe_states.shape
                        )
                        # pred_keyframe_states is only the pose
                        pred_keyframe_poses = pred_keyframe_states
                        extra_supp = torch.zeros(
                            pred_keyframe_poses.shape[0],
                            pred_keyframe_poses.shape[1],
                            self.extra_dim,
                        ...
                        before_pose_supp = torch.zeros(
                            pred_keyframe_poses.shape[0],
                            pred_keyframe_poses.shape[1],
                            self.state_dim - self.pose_dim - self.extra_dim,
                            dtype=action_history.dtype,
                            device=self.device,
                        )
                        pred_keyframe_states = torch.cat(
                            [before_pose_supp, pred_keyframe_poses, extra_supp], dim=-1
                        )

                    if self.keyframe_relative_pose or self.keyframe_eval:
                        if self.extra_dim > 0:
                            pred_pose = pred_keyframe_states[
                                ...,
                                0,
                                -self.pose_dim - self.extra_dim : -self.extra_dim,
                            ]  # Only consider the first keyframe
                            cur_pose = states[
                                ...,
                                -1,
                                -self.pose_dim - self.extra_dim : -self.extra_dim,
                    ...
                    if self.keyframe_eval and self.keyframe_relative_pose:
                        ...
                        if self.pose_dim == 6:
                            if (
                                self.using_euler
                            ):  # Transfer euler to axis-angle (action)
                                pred_pose = np.r_[
                                    pred_pose[:3], euler2axangle(*pred_pose[3:])
                                ]
                        else:  # Transfer quat to axis-angle (action)
                            pred_pose = np.r_[
                                pred_pose[:3],
                                compact_axis_angle_from_quaternion(pred_pose[3:]),
                                pred_keyframe_actions[0, 0, -1],
                            ]
                        return pred_pose[np.newaxis][
                            np.newaxis
                        ]  # Return relative pose (by axis-angle) directly
                        ...
                        if self.pose_dim == 6:
                            if self.using_euler:  # Transfer euler to quat
                                pred_pose = np.r_[
                                    pred_pose[:3],
                                    euler2quat(
                                        pred_pose[3], pred_pose[4], pred_pose[5]
                                    ),
                                ]
                                cur_pose = np.r_[
                                    cur_pose[:3],
                                    euler2quat(cur_pose[3], cur_pose[4], cur_pose[5]),
                                ]
                            else:  # Transfer angle to quat
                                pred_pose = np.r_[
                                    pred_pose[:3],
                                    quaternion_from_compact_axis_angle(pred_pose[3:]),
                                ]
                                cur_pose = np.r_[
                                    cur_pose[:3],
                                    quaternion_from_compact_axis_angle(cur_pose[3:]),
                                ]
                        pred_pose = Pose(p=pred_pose[:3], q=pred_pose[3:])
                        cur_pose = Pose(p=cur_pose[:3], q=cur_pose[3:])
                        pred_pose_at_world = (
                            cur_pose * pred_pose
                        )  # !! Compute pose in the world
                        pred_pose = np.r_[pred_pose_at_world.p, pred_pose_at_world.q]
                        if self.pose_dim == 6:
                            if self.using_euler:  # Transfer quat to euler
                                pred_pose = np.r_[
                                    pred_pose_at_world.p,
                                    quat2euler(pred_pose_at_world.q),
                                ]
                            else:
                                pred_pose = np.r_[
                                    pred_pose_at_world.p,
                                    compact_axis_angle_from_quaternion(
                                        pred_pose_at_world.q
                                    ),
                                ]
                    ...
                    if self.keyframe_eval:
                        ...
                        pred_pose = np.r_[
                            key_pose_at_ee.p,
                            compact_axis_angle_from_quaternion(key_pose_at_ee.q),
                            pred_keyframe_actions[0, 0, -1],
                        ]  # We need axis-angle as the action
                        return pred_pose[np.newaxis][np.newaxis]

                    if self.keyframe_relative_pose:
                        if self.extra_dim > 0:
                            pred_keyframe_states[
                                ...,
                                0,
                                -self.pose_dim - self.extra_dim : -self.extra_dim,
                            ] = pred_pose
                            ...
                            pred_keyframe_states[
                                ..., 0, -self.pose_dim - self.extra_dim :
                            ] = pred_pose
                    ...
                    if self.extra_dim > 0:
                        ...
                        pred_keyframe_states[..., -6:-3] = states[
                            0, 0, -6:-3
                        ].clone()  # Target pos
                        pred_keyframe_states[..., -3:] = (
                            states[0, 0, -6:-3] - pred_keyframe_states[..., -13:-10]
                        )  # Delta pos to predicted tcp

                    pred_keyframe = torch.cat(
                        [pred_keyframe_states, pred_keyframe], dim=-1
                    )
                    ...
                    pred_keyframe_states, pred_keyframe_actions = (
                        pred_keyframe[..., : self.state_dim],
                        pred_keyframe[..., -self.action_dim :],
                    )
                ...
                pred_keyframe, pred_keytime_differences = (
                    pred_keyframe[:, :, :-1],
                    pred_keyframe[:, :, -1],
                )  # split keyframe and predicted timestep
            ...
            pred_keytime_differences = np.ceil(
                self.max_horizon * pred_keytime_differences
            )
            pred_keytime_differences = np.clip(
                pred_keytime_differences.astype(int), a_min=0, a_max=None
            )
        ...
        if not self.diffuse_state:
            ...
            data_dim = (
                self.action_dim + self.pose_dim + self.extra_dim
                if self.pose_only
                else self.action_dim + self.state_dim
            )
        # data_dim = self.action_dim+self.state_dim if self.diffuse_state else self.action_dim

        if data_mask is None:
            if self.obs_as_global_cond:
                act_mask, obs_mask, data_mask = self.mask_generator(
                    (bs, self.horizon, data_dim), self.device
                )
        ...
        if action_history.shape[1] == self.horizon:
            for key in observation:
                observation[key] = observation[key][
                    :, obs_mask, ...
                ]  # obs mask is for one dimension

        obs_fea = self.obs_encoder(
            observation, ep_first_obs_dict=ep_first_obs
        ...
        if self.action_seq_len - hist_len:
            if self.diffuse_state:
                ...
                action_history = torch.cat(
                    [action_history, action_to_state_horizon_supp], dim=1
                )
                data_history = torch.cat([states, action_history], dim=-1)
                supp = torch.zeros(
                    bs,
                    self.action_seq_len - hist_len - 1,
        ...
        if self.diffuse_state and self.pose_only:
            if self.extra_dim > 0:
                ...
                data_history = torch.cat(
                    [
                        data_history[
                            ...,
                            -self.action_dim
                            - self.pose_dim
                            - self.extra_dim : -self.action_dim,
                        ],
                        data_history[..., -self.action_dim :],
                    ],
                    dim=-1,
                )  # We include extra dim for diffusion
            else:
                data_history = torch.cat(
                    [
                        data_history[
                            ..., -self.action_dim - self.pose_dim : -self.action_dim
                        ],
                        data_history[..., -self.action_dim :],
                    ],
                    dim=-1,
                    ...
                    if self.keyframe_pose_only:
                        if self.extra_dim > 0:
                            data_history[
                                ...
                                ..., -self.extra_dim - self.pose_dim : -self.extra_dim
                            ]
                        else:
                            data_history[
                                range(bs),
                                pred_keytime[:, i : i + 1],
                                -self.pose_dim - self.action_dim : -self.action_dim,
                            ] = pred_keyframe[:, i : i + 1][..., -self.pose_dim :]
                        ...
                        if self.diffuse_state:
                            data_mask[
                                range(bs), pred_keytime[:, i], : -self.action_dim
                            ] = True
                        else:
        ...
        if (
            pred_action_seq.shape[2] != self.action_dim and pred_action_seq.shape[2]
        ) != (self.action_dim + self.state_dim):
            supp = torch.zeros(
                pred_action_seq.shape[0],
                pred_action_seq.shape[1],
                self.state_dim + self.action_dim - pred_action_seq.shape[2],
    ...
    def update_parameters(self, memory, updates):
        ...
        if self.diffuse_state:
            sampled_batch = memory.sample(
                ...
                require_mask=True,
                obsact_normalizer=self.normalizer,
                keyframe_type=self.keyframe_model_type,
            )
        else:
            sampled_batch = memory.sample(
                batch_size,
        ...
        if self.diffuse_state:
            if self.pose_only:
                if self.extra_dim > 0:
                    ...
                    traj_data = torch.cat(
                        [
                            sampled_batch["normed_states"][
                                ..., -self.pose_dim - self.extra_dim :
                            ],
                            sampled_batch["normed_actions"],
                        ],
                        dim=-1,
                    )  # We preserve the tcp pose and target pos for diffusion
                else:
                    traj_data = torch.cat(
                        [
                            sampled_batch["normed_states"][..., -self.pose_dim :],
                            sampled_batch["normed_actions"],
                        ],
                        dim=-1,
                    )  # We only preserve the tcp pose for diffusion
            else:
                traj_data = torch.cat(
                    [sampled_batch["normed_states"], sampled_batch["normed_actions"]],
                    dim=-1,
                )
        else:
        ...
        if data_mask is None:
            if self.obs_as_global_cond:
                ...
                raise NotImplementedError(
                    "Not support obs not as cond! Please set obs_as_global_cond=True"
                )

        if self.train_diff_model:
            if (self.diffusion_updates is None) or (
                (self.diffusion_updates is not None)
                and updates <= self.diffusion_updates
            ):
        ...
        if self.train_keyframe_model:
            if (self.keyframe_model_updates is None) or (
                (self.keyframe_model_updates is not None)
                and updates <= self.keyframe_model_updates
            ):
                ...
                if self.keyframe_model_type == "bc":
                    keyframe_obs_fea = self.keyframe_obs_encoder(
                        masked_obs, ep_first_obs_dict=ep_first_obs
                    )

                    keyframe_states = keyframe_states[:, obs_mask, ...][
                        :, -1
                    ]  # We only take the last step of the horizon since we want to train the key frame model
                    keyframe_masks = keyframe_masks[:, obs_mask, ...][:, -1]
                    keytime_differences = keytime_differences[:, obs_mask, ...][:, -1]
                    keyframe_loss, info = self.keyframe_bc_loss(
                        keyframe_obs_fea,
                        keyframe_states,
                        keytime_differences,
                        keyframe_masks,
                    ...
                    if not self.keyframe_state_only:
                        img_obs_fea = self.keyframe_obs_encoder(
                            masked_obs,
                            ep_first_obs_dict=ep_first_obs,
                            img_fea_only=True,
                        )
                    ...
                    keyframe_loss, info = self.keyframe_gpt_loss(
                        keyframe_obs_fea,
                        timesteps,
                        actions,
                        keyframe_states,
