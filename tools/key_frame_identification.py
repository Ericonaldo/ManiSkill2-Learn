...
def keyframeDetectionByFrames(
    frames, dest, Thres, plotMetrics=False, verbose=False, suffix=""
):
    ...
    for x in indices:
        cv2.imwrite(
            os.path.join(keyframePath, "keyframe" + str(cnt) + ".jpg"), full_color[x]
        )
        cnt += 1
        log_message = (
            "keyframe " + str(cnt) + " happened at " + str(timeSpans[x]) + " sec."
        )
        if verbose:
            print(log_message)
...
def _is_stopped(
    demo, i, obs, stopped_buffer, delta=0.01, grip_thresh=0.3, use_gripper=False
):
    obss, acts = demo
    ...
    if i <= 0 or (not use_gripper):
        ...
        gripper_state_no_change = i < (len(obss) - 2) and (
            gripper_open == (acts[i][-1] > grip_thresh)
            and gripper_open == (acts[i - 2][-1] > grip_thresh)
            and (acts[i - 3][-1] > grip_thresh) == (acts[i - 2][-1] > grip_thresh)
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--control-mode", help="Control mode", type=str, default="pd_ee_delta_pose"
    )  # pd_ee_pose # pd_joint_pos
    parser.add_argument(
        "--keyframe-mode", help="Keyframe detection mode", type=str, default="joints"
    )  # video
...
if __name__ == "__main__":
    ...
    if filenames is None:
        filenames = [
            f"../ManiSkill2/demos/{args.body}_body/{args.env_id}-v0/trajmslearn.rgbd.{args.control_mode}.h5"
        ]
    ...
    for filename in filenames:
        ...
        res_file = h5py.File(
            f"../ManiSkill2/demos/{args.body}_body/{args.env_id}-v0/keyframes.{args.control_mode}.h5",
            "w",
        )
        ...
        for key in tqdm(traj_keys):
            ...
            if args.keyframe_mode == "video":
                for p in perspective:
                    tmp_frames = item["obs"][f"{p}_camera_rgbd"][:, :3, :, :].astype(
                        np.float32
                    )  # (C,H,W)
                    tmp_frames = tmp_frames.swapaxes(1, 2).swapaxes(2, 3)  # (H,W,C)
                    tmp_frame_idxes = keyframeDetectionByFrames(
                        tmp_frames,
                        f"key_frames/{args.env_id}",
                        args.threshold,
                        True,
                        True,
                        p,
            ...
            if len(frame_idxes) > 3:
                ...
                for x in frame_idxes:
                    cv2.imwrite(
                        os.path.join(
                            f"./key_frames/{args.env_id}/base/{key}", str(x) + ".jpg"
                        ),
                    ...
                    cv2.imwrite(
                        os.path.join(
                            f"./key_frames/{args.env_id}/hand/{key}", str(x) + ".jpg"
                        ),
            ...
            assert (
                min(frame_idxes) >= 0
            ), f"there are negative frame idxes!, {key}, {frame_idxes}"
